...
export function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh] px-4" onClick={onClose}>
      <div className="absolute inset-0 bg-[#2B2B2F]/50 backdrop-blur-sm" />
      <div
        className="relative w-full max-w-2xl bg-surface rounded-[1.75rem] overflow-hidden border-[3px] border-outline shadow-pop-lg page-enter"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center gap-3 px-lg h-16 border-b-2 border-outline/15">
          ...
          />
          {loading && <span className="w-4 h-4 rounded-full border-2 border-outline/20 border-t-primary animate-spin" />}
          <kbd className="text-[10px] px-2 py-1 rounded-full bg-surface-container text-on-surface-variant border-2 border-outline/30 font-bold">
            ESC
          </kbd>
        </div>
